...
void logWindowStateChange(bool newState, WindowAction action) {
  ...
  Serial.print(newState ? "APERTE" : "CHIUSE");
  Serial.print(" - ");
  
  switch (action) {
    case WINDOW_MANUAL_OPEN:
      Serial.println("Apertura MANUALE");
      break;
    case WINDOW_MANUAL_CLOSE:
      Serial.println("Chiusura MANUALE");
      break;
    case WINDOW_AUTO_OPEN:
      Serial.println("Apertura AUTOMATICA");
      break;
    case WINDOW_AUTO_CLOSE:
      Serial.println("Chiusura AUTOMATICA");
      break;
    case WINDOW_RAIN_CLOSE:
      Serial.println("Chiusura per PIOGGIA");
      break;
    case WINDOW_SENSOR_ERROR_CLOSE:
      Serial.println("Chiusura di SICUREZZA (errore sensore pioggia)");
      break;
    case WINDOW_SEND_ALL_OPEN:
      Serial.println("Apertura TOTALE (invio comando all)");
      break;
    case WINDOW_SEND_ALL_CLOSE:
      Serial.println("Chiusura TOTALE (invio comando all)");
      break;
    case WINDOW_ALL_OPEN:
      Serial.println("Apertura TOTALE (comando remoto)");
      break;
    case WINDOW_ALL_CLOSE:
      Serial.println("Chiusura TOTALE (comando remoto)");
      break;
    default:
      Serial.println("Causa SCONOSCIUTA");
      break;
  }
}

void logTFavorableChange(bool newValue, int tempIn, int tempOut) {
  Serial.print("[Thermal condition] ");
  if (newValue) {
    Serial.print("FAVOREVOLI ");
  } else {
    Serial.print("SFAVOREVOLI ");
  }
  Serial.print("(Tin="); Serial.print(tempIn);
  Serial.print("°C, Tout="); Serial.print(tempOut);
  Serial.println("°C)");
}
