// Sala2_jul13a.ino — Nano 33 IoT (SAMD21)
...
void setup() {
  ...
  Serial.println(F("[SYS] Sala2 avviata - SYS_BOOT"));
}
...
void onCloudSync() {
  ...
  snprintf(buf, sizeof(buf), "Sala2: SYNC allW:%d wPos:%d", allWindows, (int)winPos);
  messager = buf;
...
void checkResetCommand() {
  if (messager == "reset" || messager == "reset2") {
